<script lang="ts">
  ...
  $: sortedTrackers = availableTrackers.sort((a, b) => getCapabilityScore(b) - getCapabilityScore(a));
</script>
        ...
        <div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
            ...
            <div>
              ...
              <div class="flex items-center space-x-2">
                <span class="text-xs px-2 py-1 rounded-full {
                  tracker.isConnected ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'
                }">
          ...
          <div class="flex items-center space-x-2">
            ...
            <div class="flex space-x-1">
              {#each [
                { key: 'hasRecovery', label: 'Recovery', color: 'bg-blue-500' },
                { key: 'hasStrain', label: 'Strain', color: 'bg-purple-500' },
                { key: 'hasHRV', label: 'HRV', color: 'bg-green-500' },
                { key: 'realTimeData', label: 'Live', color: 'bg-orange-500' }
              ] as cap}
                {#if tracker.capabilities[cap.key]}
                  <div 
                    class="w-2 h-2 rounded-full {cap.color}" 
                    title={cap.label}
                  ></div>
                {/if}
              {/each}
    ...
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-xl p-6 max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-lg font-semibold text-gray-900">Add Fitness Tracker</h3>
          <button
          ...
          {#each sortedTrackers as tracker}
            <button
              ...
              class="p-4 border rounded-lg text-left transition-colors {
                selectedTrackerType === tracker.type
            ...
            >
              ...
              <div class="flex flex-wrap gap-1 mb-2">
                {#each [
                  { key: 'hasRecovery', label: 'Recovery', color: 'blue' },
                  { key: 'hasStrain', label: 'Strain', color: 'purple' },
                  { key: 'hasHRV', label: 'HRV', color: 'green' },
                  { key: 'realTimeData', label: 'Live', color: 'orange' }
                ] as cap}
                  {#if tracker.capabilities[cap.key]}
        ...
        <div class="mt-6 p-4 bg-gray-50 rounded-lg">
          ...
          <div class="grid grid-cols-2 gap-2 text-xs text-gray-600">
            <div><span class="font-medium text-blue-700">Recovery:</span> Daily readiness scores</div>
            <div><span class="font-medium text-purple-700">Strain:</span> Workout intensity tracking</div>
            <div><span class="font-medium text-green-700">HRV:</span> Heart rate variability</div>
            <div><span class="font-medium text-orange-700">Live:</span> Real-time data during workouts</div>
          </div>
        </div>
      </div>
    </div>
