<script lang="ts">
	...
	let recentWorkouts = [
		{
			...
			program: 'Beginner Fitness Program',
			date: '2025-02-01',
			duration: 45,
			caloriesBurned: 320,
			completed: true
	...
	function handleStartNewWorkout() {
		console.log('[WORKOUT] Starting new workout with Alice card animation');
		// Use workout store to trigger Alice card animation instead of navigation
...
<div class="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
	<div class="flex items-center justify-between mb-6">
		...
		<button 
			class="text-primary hover:text-blue-700 text-sm font-medium cursor-pointer"
	...
	<div class="space-y-4">
		{#each recentWorkouts as workout}
			<div class="flex items-center p-4 rounded-lg border border-gray-100 hover:border-gray-200 transition-colors">
				<div class="flex-shrink-0">
					<div class="w-10 h-10 rounded-lg flex items-center justify-center
						{workout.completed ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}">
