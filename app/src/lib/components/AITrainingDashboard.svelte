...
<script lang="ts">
	...
	async function loadAIInsights() {
		...
		try {
			...
			deloadRecommendation = await aiEngine.determineDeloadTiming('user1', 4, {
				trend: 'stable',
				averageRecovery: 68
			});
		} catch (error) {
	...
	{:else}
		...
		{#if currentAnalysis}
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
				...
				<div class="bg-white rounded-lg shadow-md p-6">
					...
					<div
						class="text-2xl font-bold {getPerformanceColor(Math.abs(currentAnalysis.effortTrend))}"
					>
						{getTrendIcon(currentAnalysis.effortTrend)}
						{Math.abs(Math.round(currentAnalysis.effortTrend))}%
					</div>
				...
				<div class="bg-white rounded-lg shadow-md p-6">
					...
					<div
						class="text-2xl font-bold {getPerformanceColor(
							Math.abs(currentAnalysis.recoveryTrend)
						)}"
					>
						{getTrendIcon(currentAnalysis.recoveryTrend)}
						{Math.abs(Math.round(currentAnalysis.recoveryTrend))}%
					</div>
		...
		<div class="bg-white rounded-lg shadow-md p-6">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
				...
				<div>
					<label for="set-number-select" class="block text-sm font-medium text-gray-700 mb-2"
						>Set Number</label
					>
					<select
		...
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
			...
			{#if deloadRecommendation}
				<div class="bg-white rounded-lg shadow-md p-6">
					...
					<div class="space-y-4">
						<div
							class="p-4 rounded-lg {deloadRecommendation.shouldDeload
				...
				<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
					<div class="text-center p-4 bg-blue-50 rounded-lg">
						<div class="text-2xl font-bold text-blue-600 mb-1">
							{programAdjustments.loadAdjustment > 0 ? '+' : ''}{Math.round(
								programAdjustments.loadAdjustment * 100
							)}%
						</div>
						<div class="text-sm font-medium text-gray-700">Load Adjustment</div>
						<div class="text-xs text-gray-600 mt-1">
							{programAdjustments.loadAdjustment > 0
								? 'Increase weight'
								: programAdjustments.loadAdjustment < 0
									? 'Decrease weight'
									: 'Maintain weight'}
					...
					<div class="text-center p-4 bg-green-50 rounded-lg">
						<div class="text-2xl font-bold text-green-600 mb-1">
							{programAdjustments.volumeAdjustment > 0 ? '+' : ''}{Math.round(
								programAdjustments.volumeAdjustment * 100
							)}%
						</div>
						<div class="text-sm font-medium text-gray-700">Volume Adjustment</div>
						<div class="text-xs text-gray-600 mt-1">
							{programAdjustments.volumeAdjustment > 0
								? 'Add sets/reps'
								: programAdjustments.volumeAdjustment < 0
									? 'Reduce sets/reps'
									: 'Maintain volume'}
					...
					<div class="text-center p-4 bg-purple-50 rounded-lg">
						<div class="text-2xl font-bold text-purple-600 mb-1">
							{programAdjustments.intensityAdjustment > 0 ? '+' : ''}{Math.round(
								programAdjustments.intensityAdjustment * 100
							)}%
						</div>
						<div class="text-sm font-medium text-gray-700">Intensity Adjustment</div>
						<div class="text-xs text-gray-600 mt-1">
							{programAdjustments.intensityAdjustment > 0
								? 'Increase effort'
								: programAdjustments.intensityAdjustment < 0
									? 'Decrease effort'
									: 'Maintain effort'}
			...
			<div class="space-y-3">
				<div class="p-3 bg-blue-50 rounded-lg border-l-4 border-blue-500">
					<p class="text-sm text-blue-800">
						💡 <strong>Personalization:</strong> Your RIR predictions will become more accurate as you
						complete more training sessions. The AI learns your individual fatigue patterns.
					</p>
				</div>

				<div class="p-3 bg-green-50 rounded-lg border-l-4 border-green-500">
					<p class="text-sm text-green-800">
						🎯 <strong>Auto-Calibration:</strong> The system automatically adjusts training parameters
						based on your recovery data and performance trends.
					</p>
				</div>

				<div class="p-3 bg-yellow-50 rounded-lg border-l-4 border-yellow-500">
					<p class="text-sm text-yellow-800">
						⚠️ <strong>Fatigue Monitoring:</strong> Real-time strain monitoring helps prevent overtraining
						and reduces injury risk through intelligent rest period adjustments.
					</p>
				</div>
			</div>
		</div>
