...
<script lang="ts">
	...
	async function loadWorkoutHistory() {
		try {
			...
			workoutHistory = workouts.map((workout) => ({
				...
				targetAchieved:
					(workout.averageIntensity || 0) >= 60 && (workout.averageIntensity || 0) <= 85
			}));
	...
	function calculateStats() {
		...
		stats.averageIntensity =
			filteredHistory.reduce((sum, w) => sum + w.averageIntensity, 0) / filteredHistory.length;
		...
		stats.targetAchievementRate =
			(filteredHistory.filter((w) => w.targetAchieved).length / filteredHistory.length) * 100;
		...
		if (filteredHistory.length >= 14) {
			...
			const previousAvg =
				previous.reduce((sum, w) => sum + w.averageIntensity, 0) / previous.length;
			...
			<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
				...
				<div class="p-4 bg-gray-800 rounded-lg">
					<div
						...
						class:text-green-400={stats.improvementTrend > 0}
						class:text-red-400={stats.improvementTrend < 0}
						class:text-gray-400={stats.improvementTrend === 0}
					>
								...
								<div class="relative h-full">
									{#each filteredHistory.slice(-20) as workout, i}
										<button
											...
											class:bg-orange-400={workout.averageIntensity >= 70 &&
												workout.averageIntensity < 85}
											class:bg-green-400={workout.averageIntensity >= 60 &&
												workout.averageIntensity < 70}
											class:bg-yellow-400={workout.averageIntensity >= 40 &&
												workout.averageIntensity < 60}
											class:bg-blue-400={workout.averageIntensity < 40}
											style="left: {(i / Math.max(1, filteredHistory.slice(-20).length - 1)) *
												100}%; bottom: {workout.averageIntensity}%; transform: translate(-50%, 50%)"
											on:click={() => selectWorkout(workout)}
											title="{workout.date.toLocaleDateString()}: {workout.averageIntensity.toFixed(
												1
											)}%"
										>
										</button>
								...
								{#each filteredHistory as workout, i}
									...
									>
										<div class="flex items-center justify-between">
											<div class="flex items-center gap-3">
												<div
													class="w-3 h-3 rounded-full {getIntensityBgColor(
												...
												<div>
													...
													<div class="text-sm text-gray-400">
														{workout.exerciseType} • {workout.totalSets} sets
													</div>
												</div>
											</div>
							...
							<div class="grid grid-cols-7 gap-2 text-center text-sm">
								...
								{#each Array(35) as _, day}
									<div class="aspect-square p-1">
										<div
											class="w-full h-full rounded bg-gray-700 flex items-center justify-center text-xs"
										>
											{(day % 31) + 1}
										</div>
									</div>
								...
								<div class="space-y-2 text-sm">
									...
									<div class="flex justify-between">
										...
										<span class="text-green-400"
											>{selectedWorkout.targetAchieved ? '✅ Yes' : '❌ No'}</span
										>
									</div>
								</div>
							</div>
