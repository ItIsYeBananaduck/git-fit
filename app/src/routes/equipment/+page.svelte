			...
			<div class="flex flex-wrap gap-2">
				...
				{#each equipmentCategories as category}
					<button
						...
						class="px-4 py-2 rounded-lg font-medium transition-all flex items-center gap-2 {selectedCategory ===
						category.slug
							? 'bg-primary text-white'
							: 'bg-gray-100 text-gray-700 hover:bg-gray-200'}"
					>
						...
						{#each categoryEquipment as equipment}
							...
							>
								<div class="flex items-start justify-between mb-3">
									<span
										class="px-2 py-1 rounded-full text-xs font-medium border {getCategoryBadgeColor(
											equipment.category
										)}"
									>
				...
				>
					<div class="flex items-start justify-between mb-3">
						<div class="text-gray-600">
							<EquipmentIcon type={getEquipmentIconType(equipment.name)} size="32" />
						</div>
						<span
							class="px-2 py-1 rounded-full text-xs font-medium border {getCategoryBadgeColor(
								equipment.category
