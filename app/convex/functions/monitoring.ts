...
export const recordHeartRateData = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const heartRates = updatedData.map(d => d.heartRate);
    const averageHeartRate = Math.round(heartRates.reduce((a, b) => a + b, 0) / heartRates.length);
...
function generateHeartRateRecommendations(session: any): string[] {
  ...
  // Zone analysis
  const totalTime = Object.values(session.timeInZones).reduce((a: any, b: any) => a + b, 0);
